...
const MINI_APP_URL = process.env.MINI_APP_URL; // օր. https://your-domain.com

if (!BOT_TOKEN) {
  console.error('❌ TELEGRAM_BOT_TOKEN-ը կարգավորված չէ .env ֆայլում։ Bot-ը չի գործարկվի։');
  process.exit(1);
}
if (!MINI_APP_URL) {
  console.error('❌ MINI_APP_URL-ը կարգավորված չէ .env ֆայլում (քո deploy արված backend-ի https հասցեն)։');
  process.exit(1);
}
...
// ---------------------------------------------------------------------------
// /app — ուղիղ Mini App բացող inline կոճակ (հարմար է ցանկացած պահի կանչելու համար)
// ---------------------------------------------------------------------------
bot.command('app', (ctx) => {
  ctx.reply(
...
// (Ինքը՝ Bot-ը, չաթ չի վարում. ամբողջ խելացի զրույցը կատարվում է Mini App-ի միջով,
//  որը կապվում է server.js-ի /api/chat endpoint-ին)
// ---------------------------------------------------------------------------
bot.on('text', (ctx) => {
  ctx.reply(
